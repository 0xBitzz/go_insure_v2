...
def bootstrap() -> Expr:
    ...
    return Seq(
        ...
        app.state.insurer.set(Global.current_application_address()),
    )
...
def purchase_policy(
    ...
    return Seq(
        ...
        Assert(
            ...
            txn.receiver() == app.state.insurer.get(),
        ...
        (expiration_date := abi.Uint64()).set(
            registration_date.get() + app.state.policy_expiration_date.get()
        ),
        ...
        app.state.address_to_policy[Txn.sender()].set(policy),
    )
...
def approve_claim() -> Expr:
    ...
    return Seq(
        (policy := Policy()).decode(
            app.state.address_to_policy[Txn.sender()].get()
        ),
        ...
        app.state.address_to_policy[customer_address].set(policy),
    )
